...
The 20260823 refresh was rebuilt as a compact patch stack rather than replaying
...
The upstream NeoGeo loader is unchanged. SDRAM readiness is a generic handoff
ordering requirement owned by `user_io.cpp`; no NeoGeo-specific adapter or
warning-predicate fork is approved.
...
- MagiK pre-handoff SDRAM probe update: MagiK launch-active mode keeps Main's
  ...
  stock menu poll path. It records `sdram_config_ready` with `source=cache` or
  `source=menu_probe`, or `sdram_config_unavailable` when Main cannot obtain a
  valid marker. This prevents any 8-bit handoff, including high-memory NeoGeo
  titles, from inheriting an uninitialized shared SDRAM-size cache. The
  upstream NeoGeo memory-warning predicate remains unchanged.
...
- Structured launch behavior must preserve path launches and prove that virtual
  ...
  `.mgl` descriptor.
- SDRAM handoff behavior must prove cache-marker decoding for every stock size
  code, cached values avoid a second hardware probe, invalid responses remain
  invalid, and both MagiK handoff paths ensure configuration before
  `fpga_load_rbf`. Device acceptance must launch Metal Slug 3 plus another
  installed high-memory NeoGeo title, retain the readiness event, and confirm
  correct title/attract-mode graphics without the upstream memory warning.
